...
rl.on('line', function(line) {
  ...
  for(var i=0;i<lineRecords.length;i++){
    ...
    { 
      ...
      if(lineRecords[i]=="\"Population (Millions) - 2010\"")
      {
        index_population2010 = i;
        header[1]=lineRecords[i].split("\"")[1];
      }
      if(lineRecords[i]=="\"Population (Millions) - 2011\"")
      {
        index_population2011 = i;
        header[2]=lineRecords[i].split("\"")[1];
      }
      if(lineRecords[i]=="\"Population (Millions) - 2012\"")
      {
 ...
 isHeader=false;
 fs.writeFileSync("data3a.json",JSON.stringify(jsonData),encoding="utf8");
});